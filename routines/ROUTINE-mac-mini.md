# Economist daily brief — LOCAL routine (Mac mini)
...
| **You — Mac mini** | **05:45**, retry 08:00 / 10:00 | **`brief:DATE:local`** | `economist` only |
| MacBook | 06:15 | `brief:DATE:local` | your understudy |
...
```
Non-empty → the MacBook already did it. **Stop. Report success.** Do not redo it better.
...
Your lock is **`briefclaim:DATE:local`**, shared with the MacBook and nobody else. Never touch
...
```
**201** → yours, continue. **409** → the MacBook holds it; steal only if genuinely dead:
```bash
curl -s -X PATCH \
...
> lock, write nothing, and report that the Economist session needs re-authenticating so he knows to
> fix it. The cloud's brief covers his morning.
...
rather than inventing a stemmer. Expect 40–60 new per language; 250+ means the stop-list is too
...
alone. On failure leave yours `running` and **never delete it** — the MacBook finds it stale at
06:15 and finishes. Deleting on failure is what lets two machines start at once.
...
Same Step 0 check, so on a normal day you look, find the edition already there, and exit in one
call. **That is the design working, not a wasted run.**
